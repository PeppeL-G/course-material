...
4. When you are done and have pushed your branch to the remote repository, create a pull request all need to approve to merge it into the `master` branch.
...
Deciding how you should communicate with each other/which communication channels to use when is something you need to agree on in the beginning of the project too, preferably at the very first meeting you have, so everybody knows how to contact the others.
...
When you have a Teamwork Environment setup, you are ready to start implementing the application in the project. It's usually a bad idea that the entire team work on one and the same problem. For example, if you are 4 team members and you need to design your database, it's enough if just one team member works on that; there's no need for the other 3 team members to work on this task too. If they do, they'll most likely just sit and watch and waste their time instead of contributing to the project. So, to be efficient, it's crucial to divide the work among the team members.
